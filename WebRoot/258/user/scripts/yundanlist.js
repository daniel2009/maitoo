...
function initGrid() {
    ...
    jQuery(addressgrid_selector).jqGrid({
        ...
        height: "auto",
        mtype: "get",
        colNames: ['订单号', "货物品名", '发件人', '收件人', "收件地址","创建日期", "运费", "路由状态", "支付状态", "运费", "操作"],
        colModel: [
            {
            ...
            { name: 'stateStr', index: 'stateStr', width: 100,formatter: function (cellvalue, options, row) {
                var text = "";
            ...
            { name: 'payornot', index: 'payornot', width: 70,formatter: function (cellvalue, options, row) {
                var text = "";
                if(cellvalue=="0")
                {
                	text="未付款";
            ...
            {
                name: 'op', index: 'op', width: 160, formatter: function (cellValue, option, row) {
                    //返回修改按钮
                	var pay="";
                 ...
                 var detail = "<button style='margin:1px 0px 0px 5px;padding:0px 3px;width:60px' class='btn btn-purple btn-sm' type='button'  onclick=\"showdetail(" + row.id + ")\"  ><span class='fa fa-times'></span>详情</button>";
                	var print = "<button style='margin:1px 0px 0px 5px;padding:0px 3px;width:80px' class='btn btn-purple btn-sm' type='button'  onclick=\"a4Print_user(" + row.id + ")\"  ><span class='fa fa-print'></span>A4打印</button>";
                    return pay + detail+print;
                }
            }
        ],
...
function initPhoneGrid() {
    ...
    jQuery(addressgrid_selector).jqGrid({
        ...
        height: "auto",
        mtype: "get",
        colNames: ['订单号', '收件人', "货物品名","路由状态", "操作"],
        colModel: [
            {
                name: 'orderId', index: 'orderId', width: 150, formatter: function (cellvalue, options, row) {
                    return '<a href="javascript:void(0)" onclick="showdetail(' + row.id + ')">' + cellvalue + '</a>';
                }
            },
            ...
            {
                name: 'detail', index: 'detail', width: 80, formatter: function (cellvalue, options, row) {
                    var text = "";
                    if(row.detail!=null)
                    {
	                    for (var i = 0; i < row.detail.length; i++) {
...
function a4Print_user(id) {
    var iframeid = "iframePrint_userA4";
    top.window.open("printpageA4.html?id=" + id + "&ran=" + Math.random());
    //$("#" + iframeid).attr("src", "").attr("src", "printpageA4.html?id=" + id + "&ran=" + Math.random());
   // beginPrintA4(iframeid);
}

function countmorder()
{
		...
		success : function(response) {
			...
			if ("200" == code) {
				...
				if (response.data != null) {
					setSpanText("#count_morder", response.data);
					//$("#m_nopayQ").html(response.data.nopayQ);
