					...
					if(response.data!=null)
					{
						...
						if(response.data.detail!="")
						{
							...
							$.each(response.data.detail,function(){
								if(commudityinfo=="")
								{
									commudityinfo=this.productName+"*"+this.quantity;
								}
								else
								{
...
function initGrid(orderId) {
    ...
    jQuery(addressgrid_selector).jqGrid({
        ...
        colModel: [
            ...
            {
                name: 'state', index: 'state', width: 60, formatter: function (cellvalue, options, row) {
                	if(removenull(row.date)==""&&removenull(row.returnurl)!="")
                	{
                		return "快递公司："+removenull(row.thrid_pns);
                	}
                	else
                	{
                		return removenull(row.state);
                	}
                    
                }
            },
            { name: 'stateRemark', index: 'stateRemark', width: 120,  formatter: function (cellvalue, options, row) {
            	if(removenull(row.date)==""&&removenull(row.returnurl)!="")
            	{
            		return "运单号："+removenull(row.thrid_no);
            	}
            	else
            	{
            		return removenull(row.stateRemark);
            	}
                
            }},
             { name: 'remark', index: 'remark', width: 120, formatter: function (cellvalue, options, row) {
            	if(removenull(row.date)==""&&removenull(row.returnurl)!="")
            	{
            		return "<a target='_blank' href='"+row.returnurl + "'>查看路由信息" + "</a></td>";
            	}
            	else
            	{
            		return removenull(row.remark);
            	}
