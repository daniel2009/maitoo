...
function initSenderGrid() {
    ...
    senderGrid = jQuery(sendergrid_selector).jqGrid({
    ...
    $.ajaxEx({
        ...
        success: function (response) {
            ...
            if (code == "200") {
                ...
                setInputData("#senderInfo_detail", response.data);
                $("#senderInfo_detail").find("input[name='name']").val(removenull(response.data.realName));
            ...
            if (code == '200') {
                if (response.data != null) {
                   ...
                   // loadChannelSelectOptionbyuser();
                    ...
                    $("select[name='storelists']").change(loadChannelSelectOptionbyuser);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if ("200" == code) {
                ...
                $("select[name='channellist']").change(loadCommoditySelectOptionbyuser);
...
//提交网上置单
//提交运单处理
function submitOnlineOrder(isPrint) {
    ...
    if (parseFloat(parseFloat(allcvalue)) < 0) {
        alert('\"价值\"输入错误！');
        return false;
    }
    ...
    httpRequest.onreadystatechange = function () {
        if (4 == httpRequest.readyState) {
            if (200 == httpRequest.status) {
                ...
                if (obj.code == "200") {
                	if(removenull(obj.message)=="")
                	{
                		alert("提交成功！");
                	}
                	else
                	{
                		alert("提交成功！你的运单号是："+obj.message);
                	}
                    if (isPrint=="1") {
                        printYundan();
                    } else if (isPrint=="2") {
...
function printYundan() {
    //m_order/search_online_byUser?rows=1&page=1
    //data.datas.data
    $.ajax({
        type: "get",
        url: "/m_order/search_online_byUser?rows=1&page=1",//获取最新提交的一条运单
        success: function (response) {
            var code = response.code;
            if (code == "200") {
                var row = response.data.datas[0];
                if (row.id) {
                    window.open("printpageA4.html?id=" + row.id + "&ran=" + Math.random());
                }
            } else if ("607" == code) {
                alert("您尚未登录或登录已失效！");
                userlogout();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (code == "200") {
                var row = response.data.datas[0];
                if (row.id) {
                    window.open("printpage64.html?id=" + row.id + "&ran=" + Math.random());
                }
            } else if ("607" == code) {
                alert("您尚未登录或登录已失效！");
                userlogout();
	 ...
	 $.ajax({
	        ...
	        success: function (response) {
	            ...
	            if ("200" == code) {
	            	...
	            	loadChannelSelectOptionbyuser_first(response.data);
	            } else if ("607" == code) {
	                alert("您尚未登录或登录已失效！");
	                userlogout();
	            }else
	            {
	            	loadChannelSelectOptionbyuser_first(null);
...
//首次装载时，调用到默认参数
function loadChannelSelectOptionbyuser_first(setobject) {
	
	if(setobject!=null&&(removenull(setobject.z_store)!="")&&removenull(setobject.z_store)!="-1")
	{
		$("select[name='storelists']").val(setobject.z_store);
	}
        ...
        success: function (response) {
            ...
            if ("200" == code) {
                ...
                if (response.data != null) {
                    ...
                    if(setobject!=null&&(removenull(setobject.z_cid)!="")&&removenull(setobject.z_cid)!="-1")
                	{
                    	$("select[name='channellist']").val(setobject.z_cid);
                	}
