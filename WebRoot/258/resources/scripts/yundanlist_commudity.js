...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        ...
        mtype: "get",
        postData: postData,
        colNames: ['运单号', '商品详情', '收件人', '收件人地址', '状态', '总价钱', "重量"],
        colModel: [
            ...
            {
                name: 'detail', index: 'detail', width: 250, formatter: function (cellvalue, options, row) {
                    var text = "";
                    if (row.detail != null) {
                        for (var i = 0; i < row.detail.length; i++) {
                            if (text == "") {
                                text ="<span style='color:blue'>"+ row.detail[i].productName + "</span>*"+ row.detail[i].brands + "*"+ row.detail[i].quantity;
                            }
                            else {
                                text = text + "<br/><span style='color:blue'>" + row.detail[i].productName + "</span>*"+ row.detail[i].brands + "*" + row.detail[i].quantity;
                            }
                        }
                    }
                    return text;
