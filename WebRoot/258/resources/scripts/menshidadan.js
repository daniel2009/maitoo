...
function submitwanshanOrder() {
    ...
    if ((sorderid == "") && (worderid == ""))//两个不能同时为空
    {
        alert("运单号被清空，无法识别！");
        return false;
    }
    if (sorderid != "") {
        submitmsorder(type, 2);//提交上网收货
    }
    else {
        submitmsorder(type, 1);//提交网上置单
    }
}
...
//获取在线置单的运单号得到的数据进行修改,orderId是运单号
var onlineorder_postData = {};
function getonlineMorderbyOrderId(orderId, type) {
    $.ajax({
        type: "get",
        url: "/admin/m_order/get_one_online",
        data: {
            "orderId": orderId
        },
        success: function (
            ...
            if (code == "200") {
                if (response.data != null) {
                    ...
                    if (type == "1") {
                        $("#searchshangmenOrder").val(response.data.orderId);
                        $("#print_online_order strong").html("提交上门取件");
                    }
                    else if (type == "0") {
                        $("#searchonlineOrder").val(response.data.orderId);
                        $("#print_online_order strong").html("提交网上置单");
                    }
                    else {
                        alert("类型设置错误!");
                        return false;
                    ...
                    if (response.data.user == null) {
                        ...
                        $("#senderTableInfo").find(":text[name='id']").val(removenull(response.data.user.id));//用户id号
                        $("#senderTableInfo").find(":text[name='usermoney']").val("$" + removenull(response.data.user.usdBalance) + "/￥" + removenull(response.data.user.rmbBalance));//公司名称
                        var infostr = "姓名：" + response.data.user.realName + " 电话：" + response.data.user.phone + " id号：" + response.data.user.id + " 会员类型：" + getusertype(response.data.user.type);
                        $("#onlineorderuserinfo").html(infostr);
                    }
                    //装载接收人
            ...
            } else if ("40000" == code)//不是本门店的在线置单
            {
                if (confirm("不是本门店的在线置单,是否继续？    注意：如果确认修改，请注意修改渠道和商品类型！")) {
