...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        url: "/admin/gonggao/search",
        //url: "testdata/newsdata.html",
        datatype: "json",
        height: "auto",
        mtype: "get",
        //新闻编号	新闻标题	新闻内容	发布人	添加时间	修改时间	备注	操作
        colNames: ['公告编号', '公告标题', '公告内容', '发布人', "添加时间", "修改时间", "备注", "操作"],
...
function update(id) {
    ...
    if (row.img != undefined && row.img != "") {
        $("#picA").attr("href", row.img);
        $("#picA img").attr("src", row.img);
    }
    else
    {
    	$("#picA").attr("href", "");
...
function del(id) {
    ...
    window.confirmDialog("确定删除选中的公告?", "删除提示", function () {
        $.ajax({
            type: "post",
            url: "/admin/gonggao/del",
            data: idData,
            success: function (response) {
                var code = response.code;
                if (code == "200") {
