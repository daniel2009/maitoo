...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        ...
        mtype: "get",
        postData: postData,
        colNames: ['运单号', '操作人', '发件人', '收件人', '收件人地址', '状态', '类型', '支付方式', '总价钱', "重量", "创建时间", "操作"],
        colModel: [
            ...
            { name: 'employeeName', index: 'employeeName', width: 60, hidedlg: true, hidden: true },//操作员工名称
            {
...

            {
             ...
             {
                 name: 'operation', index: 'operation', width: 260, //align: "center",
                 formatter: function (cellvalue, options, row) {
                     //返回修改按钮
                     var edit = "";
                    ...
                    var copy = "<a style='width:60px;margin-left:5px;margin-top:1px;' class='btn btn-danger  btn-sm'  onclick='copy(" + row.id + ")' ><span class='icon-edit'></span>复制</a>";
                     return edit + a4Print + fhot4x6 + haiguanPrint+copy;
                 }
             },
        ],
        jsonReader: {
...
function copy(id) {
    window.location.href = "menshidadancopy.html?id="+id;
}
