...
function initGrid() {
    ...
    jQuery(addressgrid_selector).jqGrid({
        url: "/m_order/search_tran_byUser",
        datatype: "json",
        height: "auto",
        mtype: "get",

        colNames: ['运单号','转运单号', '收件人', '收件地址', "货物品名", "创建日期", "操作"],
        colModel: [
                    ...
                    if (row.detail != null) {
                        for (var i = 0; i < row.detail.length; i++) {
                            if (text == "") {
                                ...
                                text = text + "; " + row.detail[i].productName + "*" +row.detail[i].brands+"*"+ row.detail[i].quantity;
                            }
                        }
                    }
                    return text;
