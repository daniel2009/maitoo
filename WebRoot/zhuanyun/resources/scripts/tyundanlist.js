...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        ...
        mtype: "get",
        postData: postData,
        colNames: ['运单号', '转运单号','运单仓位', '会员', '收件人', '收件人地址', '状态', '类型', '支付方式', '总价钱', "重量", "创建时间", "操作"],
        colModel: [
            ...
            {
                name: 'torderIds', index: 'torderIds', width: 100,//转运单号信息

                formatter: function (cellvalue, options, row) {
                    if (row.torders != null)//转运单号
                    {
                    	var str="";
                    	$.each(row.torders,function(){
                    		...
                    		{
                    			str=this.torderId;
                    			if(this.Shelves_position!=null)
                    			{
                    				str=str+" | "+this.position.position;
                    			}
                    		}
                    		else
                    		{
            ...
            {
                name: 'positionId', index: 'positionId', width: 40,//运单仓位

                formatter: function (cellvalue, options, row) {
                    if (row.sposition != null)//转运单号
                    {
                    	return row.sposition.position;
                    }
                    else {
                        return "";
                    }
            ...
            {
                name: 'raddress', index: 'ruserId', width: 105,//收件人地址
            ...
            {
                name: 'type', index: 'type', width: 80, hidedlg: true, hidden: true,
                formatter: function (cellvalue, options, row) {
...

            {
             ...
             { name: 'createDate', index: 'createDate', width: 60 },//创建时间
             {
                 ...
                 formatter: function (cellvalue, options, row) {
                     ...
                     if ((row.state < 2 || (row.payornot != "1"))&&row.state!="-9") {
                   ...
                   //  var haiguanPrint = "<a style='width:85px;margin-left:5px;margin-top:1px;' class='btn  btn-primary btn-sm'  onclick=\"haiguanPrint(" + row.id + ")\"  ><span class='icon-print'></span> 海关(4X6)</a>";
                     ...
                     return edit + a4Print + fhot4x6; //+ haiguanPrint;
                 }
             },
        ],
        jsonReader: {
...
function showdetail(id) {
    ...
    if(row.sposition!=null)
    {
    	$("span[name='spositionId']").html(row.sposition.position);
    }
    else
    {
    	$("span[name='spositionId']").html("");
    }
    ...
    if(row.torders!=null)
    {
    	...
    	$("span[name='torderIds']").html(str);
    }
    else
    {
    	$("span[name='torderIds']").html("");
    }
}
