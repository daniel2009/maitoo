...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        url: "/admin/m_order/search_nopay",
            ...
            { name: 'totalmoney', index: 'totalmoney', width: 40 },
             ...
             { name: 'createDate', index: 'createDate', width: 80 },//创建时间
             {
                 ...
                 formatter: function (cellvalue, options, row) {
                     ...
                     if ((row.payornot != "1")) {
                    	 pay = "<a style='width:60px;margin-left:5px;margin-top:1px;' class='btn btn-danger  btn-sm'  onclick='paybyadmin(" + row.id + ")' ><span class='icon-edit'></span>支付</a>";
                     }
                     return pay;
                 }
             },
