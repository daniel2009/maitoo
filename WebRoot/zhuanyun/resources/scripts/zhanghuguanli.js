...
function initZhanghuList() {
    jQuery(yundangrid_selector).jqGrid({
        ...
        colNames: ['用户编号','电话', '姓名','邮箱', '会员等级','冻结金额','余额(rmb/usd)', '充值金额', '货币类型', "备注", "操作"],
        ...
        colModel: [
            ...
            {
                name: 'freeze', index: 'freeze', width: 80, formatter: function (cellValue, option, row) {
               	 
               	 if(removenull(row.freezeMoney)!=""&&row.freezeMoney.length>0)
               	 {
               		 var rmb=0;
               		 var usa=0;
               		 $.each(row.freezeMoney,function(){
               			 if(removenull(this.rmb)!=""&&!isNaN(parseFloat(this.rmb)))
               			 {
               				 rmb+=parseFloat(this.rmb);
               			 }
               			 if(removenull(this.usa)!=""&&!isNaN(parseFloat(this.usa)))
               			 {
               				 usa+=parseFloat(this.usa);
               			 }
               		 });
...

            {
             ...
             {
                 name: 'op', index: 'op', width:140, formatter: function (cellValue, option, row) {
                     ...
                     var clearfreeze= "<button style='margin-left:5px;margin-top:1px;' class='btn btn-danger btn-sm' type='button'  onclick=\"clearfreeze('" + row.id + "')\"  ><span></span>清空冻结</button>";
                     return chongzhi+clearfreeze;
                 }
             }
        ],
...
function chongzhi(id) {
    ...
    var currencytext = $(yundangrid_selector).find("[id=" + id + "]").find("select[name='currency'] option:selected").text();
    if(confirm("确定给客户充值"+data.realAmount+currencytext+"?"))
    {}
    else
    {
    	return false;
...
function clearfreeze(id)
{
	  var data = {};
	    data.userId = id;
	    
	    if(confirm("确定清空用户"+id+"的冻结余额?"))
	    {}
	    else
	    {
	    	return false;
