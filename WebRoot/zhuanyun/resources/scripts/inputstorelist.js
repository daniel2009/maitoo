...
$(function ($) {
    ...
    jQuery(yundangrid_selector).jqGrid({
        url: "/admin/m_order/search_input",
        //url: "testdata/yundanlistdata.html",
        datatype: "json",
        height: "auto",
        mtype: "get",
        postData: postData,
        colNames: ['运单号', '入库门店', '所属门店', '状态', '支付方式', '总价钱', '成本',"计费重量", "实际重量","入库重量","入库计费重量", "操作员工", "入库时间","异常备注"],
        colModel: [
            ...
            { name: 'i_storeName', index: 'i_storeName', width: 60 },//入库的门店
            { name: 'storeName', index: 'storeName', width: 60 },//所属门店
            {
            ...
            {
                name: 'payWay', index: 'payWay', width: 60,hidedlg:true,hidden:true,
                formatter: function (cellvalue, options, row) {
                    if (cellvalue == "1") {
                        return "现金支付";
                    } else if (cellvalue == "0") {
                        ...
                        return "<span style='color:red'>非法支付</span>";
                    }
                }
            },
             { name: 'totalmoney', index: 'totalmoney', width: 40 },
             { name: 'totalcost', index: 'totalcost', width: 40 },
             { name: 'weight', index: 'weight', width: 40 },
             { name: 'sjweight', index: 'sjweight', width: 40 },
             { name: 'i_weight', index: 'i_weight', width: 40 , formatter: function (cellvalue, options, row) {
            	 if(!isNaN(parseFloat(row.i_weight))&&!isNaN(parseFloat(row.sjweight)))
            	 {
            		 //误差大于等于0.5
            		 if(parseFloat(row.i_weight)<=0||parseFloat(row.sjweight)<=0)
            		 {
            			 return "<span style='color:red'>"+row.i_weight+"</span>";
            		 }
            		 else if(Math.abs(parseFloat(row.i_weight)-parseFloat(row.sjweight))>=0.5)
            		 {
            			 return "<span style='color:darkcyan'>"+row.i_weight+"</span>";
            		 }
            		 else
            		 {
            			 return row.i_weight;
            	 ...
            	 {
            		 return "<span style='color:blue'>"+row.i_weight+"</span>";
            	 }
            	 
             }},
             
             { name: 'i_jwweight', index: 'i_jwweight', width: 40 , formatter: function (cellvalue, options, row) {
            	 
            	 //保存进位重量
            	 if(!isNaN(parseFloat(row.i_weight)))
            	 {
            		 //误差大于等于0.5
            		 //进位数据正常
            		if(!isNaN(parseFloat(jw_commodity_rate))&&!isNaN(parseFloat(lowest_weight_value_flag)))
            		{
            			if(parseFloat(jw_commodity_rate)<=0||parseFloat(lowest_weight_value_flag)<=0)
            			{
            				return "";
            			}
            			else
            			{
            				if(jw_commodity_rate>0&&jw_commodity_rate<1)//进位值正常
            				{
            					if(parseFloat(row.i_weight)<=parseFloat(lowest_weight_value_flag))
            					{
            						return lowest_weight_value_flag;
            					}
            					else
            					{
            						if((parseFloat(row.i_weight)-parseInt(row.i_weight))<=parseFloat(jw_commodity_rate))
            						{
            							return parseInt(row.i_weight);
            						}
            						else
            						{
            							var temp=parseInt(row.i_weight)+1;
            							return  temp;
            						}
            					}
            				}
             ...
             { name: 'i_employeeName', index: 'i_employeeName', width: 40 },//入库员工
             { name: 'i_date', index: 'i_date', width: 60 },//入库时间
             {
            	 //标注可能发生的异常
                 name: 'exceptionq', index: 'exceptionq', width: 60,
                 formatter: function (cellvalue, options, row) {
                	 var str="";
                     if (isNaN(parseFloat(row.state))) {
                         str="状态异常";
                     } else if (parseFloat(row.state)<2) {
                         return "属于揽收之前的状态";
                     } else if (parseFloat(row.state)>9) {
                         return "状态超出范围";
                     } else {
                     ...
                     } else if ((parseFloat(row.weight))<=0) {
					    	if(str=="")
					    	{
					    		 str+="计费重量小于等于0";
					    	}
                     ...
                     if (isNaN(parseFloat(row.sjweight))) {
                    	 if(str=="")
                    	{
                    		 str+="实际重量异常";
                    	}
                     ...
                     } else if ((parseFloat(row.sjweight))<=0) {
					    	if(str=="")
					    	{
					    		 str+="实际重量小于等于0";
					    	}
                     ...
                     } else if ((parseFloat(row.i_weight))<=0) {
					    	if(str=="")
					    	{
					    		 str+="入库重量小于等于0";
					    	}
                     ...
                     if (isNaN(parseFloat(row.totalmoney))) {
                    	 if(str=="")
                    	{
                    		 str+="总费用异常";
                    	}
                     ...
                     } else if ((parseFloat(row.totalmoney))<=0) {
					    	if(str=="")
					    	{
					    		 str+="总费用小于等于0";
					    	}
                     ...
                     if (isNaN(parseFloat(row.totalcost))) {
                    	 if(str=="")
                    	{
                    		 str+="成本异常";
                    	}
                     ...
                     //alert(row.payway);
                     if(row.payWay=="0"||row.payWay=="1"||row.payWay=="2")
                     {
      ...
      // $(":hidden[name='checked_ids']").val(ids);
        ...
        // var totalmoney=0;
	           ...
	           for(var i=0;i<ids.length;i++)
	           {
        			...
        			else
        			{
        				if(jw_commodity_rate>0&&jw_commodity_rate<1)//进位值正常
        				{
        					...
        					else
        					{
        						if((parseFloat(row.i_weight)-parseInt(row.i_weight))<=parseFloat(jw_commodity_rate))
        						{
        							jwweight= parseInt(row.i_weight);
        						}
        						else
        						{
        							var temp=parseInt(row.i_weight)+1;
	            	...
	            	if(row.payWay=="0")//余额支付
	            	{
	            		...
	            		if(isNaN(parseFloat(row.totalmoney))||parseFloat(row.totalmoney)<=0)
	            		{
	            			...
	            			all_wrong_weight+=parseFloat(row.weight);//所有错误计费重量
	                       all_wrong_sjweight+=parseFloat(row.sjweight);//所有错误实际重量
	                       all_wrong_i_weight+=parseFloat(row.i_weight);//所有错误入库重量
	                       all_wrong_cost+=parseFloat(row.totalcost);//所有错误成本
	                       all_wrong_totalmoney+=parseFloat(row.totalmoney);//所有总价钱
	            		...
	            		else
	            		{
		            		...
		            		else
		            		{
			            		...
			            		else
			            		{
				            		...
				            		{
				            			if(isNaN(parseFloat(row.i_weight))||parseFloat(row.i_weight)<=0)
					            		{
					            			pay_ye_q_no++;
					            			allquestion++;
					            			all_wrong_weight+=parseFloat(row.weight);//所有错误计费重量
					            		...
					            		{
					            			pay_ye_cost+=parseFloat(row.totalcost);
					            			pay_ye_money+=parseFloat(row.totalmoney);
					            			pay_ye_weight+=parseFloat(row.weight);
					                        pay_ye_sjweight+=parseFloat(row.sjweight);
					                        pay_ye_i_weight+=parseFloat(row.i_weight);
					                        allweight+=parseFloat(row.weight);
					                        allsjweight+=parseFloat(row.sjweight);
					                        all_i_weight+=parseFloat(row.i_weight);
		            		...
		            		else
		            		{
			            		...
			            		else
			            		{
				            		...
				            		else
				            		{
					            		...
					            		{
					            			pay_cash_cost+=parseFloat(row.totalcost);
					            			pay_cash_money+=parseFloat(row.totalmoney);
					            			pay_cash_weight+=parseFloat(row.weight);
					                        pay_cash_sjweight+=parseFloat(row.sjweight);
					                        pay_cash_i_weight+=parseFloat(row.i_weight);
					                        allweight+=parseFloat(row.weight);
					                        allsjweight+=parseFloat(row.sjweight);
					                        all_i_weight+=parseFloat(row.i_weight);
		            		...
		            		else
		            		{
			            		...
			            		else
			            		{
				            		...
				            		else
				            		{
					            		...
					            		{
					            			pay_self_cost+=parseFloat(row.totalcost);
					            			pay_self_money+=parseFloat(row.totalmoney);
					            			pay_self_weight+=parseFloat(row.weight);
					                        pay_self_sjweight+=parseFloat(row.sjweight);
					                        pay_self_i_weight+=parseFloat(row.i_weight);
					                        allweight+=parseFloat(row.weight);
					                        allsjweight+=parseFloat(row.sjweight);
					                        all_i_weight+=parseFloat(row.i_weight);
		            		...
		            		else
		            		{
			            		...
			            		else
			            		{
				            		...
				            		else
				            		{
					            		...
					            		{
					            			morder_nopay_cost+=parseFloat(row.totalcost);
					            			morder_nopay_totalmoney+=parseFloat(row.totalmoney);
					            			morder_nopay_weight+=parseFloat(row.weight);
					            			morder_nopay_sjweight+=parseFloat(row.sjweight);
					            			morder_nopay_i_weight+=parseFloat(row.i_weight);
	           ...
	           $("#morder_all_weight").html(temp);
	           temp=parseFloat(allsjweight)+parseFloat(all_wrong_sjweight);
	           $("#morder_all_sjweight").html(temp);
	           temp=parseFloat(all_i_weight)+parseFloat(all_wrong_i_weight);
	           $("#morder_all_i_weight").html(temp);
	           temp=parseFloat(all_i_jwweight)+parseFloat(all_wrong_i_jwweight);
	           $("#morder_all_i_jwweight").html(temp);
	           
	           
	           $("#morder_weight").html(allweight);
	           $("#morder_sjweight").html(allsjweight);
	           
	           $("#morder_pay_way_q").html(pay_way_wrong);
	           $("#morder_all_q").html(allquestion);
	           
	           $("#morder_i_weight").html(all_i_weight);
	           $("#morder_i_jwweight").html(all_i_jwweight);//正确的进位重量
	           $("#morder_right_num").html(morder_right_num);//正确
	           
	           
	           //错误运单总统计
	          $("#all_wrong_weight").html(all_wrong_weight);
	          $("#all_wrong_sjweight").html(all_wrong_sjweight);
	          $("#all_wrong_i_weight").html(all_wrong_i_weight);
	          $("#all_wrong_cost").html(all_wrong_cost);
	       	  $("#all_wrong_totalmoney").html(all_wrong_totalmoney);
	       	  $("#all_wrong_i_jwweight").html(all_wrong_i_jwweight);
	       	
	     
	          
	           
	           
	           $("#morder_pay_ye_no").html(pay_ye_no);
	           $("#morder_pay_ye").html(pay_ye_money);
	           $("#morder_pay_ye_cost").html(pay_ye_cost);
	           $("#morder_pay_ye_weight").html(pay_ye_weight);
	           $("#morder_pay_ye_sjweight").html(pay_ye_sjweight);
	           $("#morder_pay_ye_i_weight").html(pay_ye_i_weight);
	           $("#morder_pay_ye_i_jwweight").html(pay_ye_i_jwweight);
	           $("#morder_pay_ye_q").html(pay_ye_q_no);
	           
	           $("#morder_pay_cash_no").html(pay_cash_no);
	           $("#morder_pay_cash").html(pay_cash_money);
	           $("#morder_pay_cash_cost").html(pay_cash_cost);
	           $("#morder_pay_cash_weight").html(pay_cash_weight);
	           $("#morder_pay_cash_sjweight").html(pay_cash_sjweight);
	           $("#morder_pay_cash_i_weight").html(pay_cash_i_weight);
	           $("#morder_pay_cash_i_jwweight").html(pay_cash_i_jwweight);
	           $("#morder_pay_cash_q").html(pay_cash_q_no);
	           
	           $("#morder_pay_self_no").html(pay_self_no);
	           $("#morder_pay_self").html(pay_self_money);
	           $("#morder_pay_self_cost").html(pay_self_cost);
	           $("#morder_pay_self_weight").html(pay_self_weight);
	           $("#morder_pay_self_sjweight").html(pay_self_sjweight);
	           $("#morder_pay_self_i_weight").html(pay_self_i_weight);
	           $("#morder_pay_self_i_jwweight").html(pay_self_i_jwweight);
	           $("#morder_pay_self_q").html(pay_self_q_no);
	           
	           
	           
	           
	           $("#morder_nopay_no").html(morder_nopay_no);
	           $("#morder_nopay_totalmoney").html(morder_nopay_totalmoney);
	           $("#morder_nopay_cost").html(morder_nopay_cost);
	           $("#morder_nopay_weight").html(morder_nopay_weight);
	           $("#morder_nopay_sjweight").html(morder_nopay_sjweight);
	           $("#morder_nopay_i_weight").html(morder_nopay_i_weight);
	           $("#morder_nopay_i_jwweight").html(morder_nopay_i_jwweight);
	           $("#morder_nopay_q_no").html(morder_nopay_q_no);
       ...
       onSelectAll: function (aRowids, status) {
             ...
             // var totalmoney=0;
     	           ...
     	           //汇总信息         
     	           $("#morder_all_weight").html(parseFloat(allweight)+parseFloat(all_wrong_weight));
     	           $("#morder_all_sjweight").html(parseFloat(allsjweight)+parseFloat(all_wrong_sjweight));
     	           $("#morder_all_i_weight").html(parseFloat(all_i_weight)+parseFloat(all_wrong_i_weight));
     	           $("#morder_all_i_jwweight").html(parseFloat(all_i_jwweight)+parseFloat(all_wrong_i_jwweight));
...
//支付运单要先核实价格
	if(pay==1)
	{
		...
		var url = "/admin/m_order/modify_check_price";
				...
				if(200 == httpRequest.status){
					...
					if(obj.code=="200")
					{
						...
						if (confirm("核实价格: "+obj.data+"美元,是否继续？          "+message)) {
							...
							var url1 = "/admin/m_order/list_modify_order";
	...
	else
	{
		...
		formData.append("payflag", "0");//修改运单
		var httpRequest1 = new XMLHttpRequest();
		var url1 = "/admin/m_order/list_modify_order";
		httpRequest1.onreadystatechange = function(){
			if(4 == httpRequest1.readyState){
        ...
        success: function (response) {
            ...
            if ("200" == code) {
                ...
                if (response.data != null) {
                    ...
                    $("select[name='inputstore']").html(str);
...
function printinputmorders()
{
     ...
     $("#iframePrintinput").attr("src", "printpageinputlist.html?id="+ids+"&ran="+Math.random());
     return false;
...
function download_inputordersbyorderIds_form()
{
	if (confirm("确定下载搜索条件下的运单？")) {
		return true;
	}
