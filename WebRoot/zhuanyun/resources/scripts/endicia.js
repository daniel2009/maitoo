...
function print_Endicia_label()
{
	...
	var userId=$("#userIdinfo [name=userId]").val();
	...
	$.ajax({
		post : "post",
		url : "/admin/endicial/admin_check_price",
		data : {
		"userId":userId,
		"labelType" : labelType,
		"dateAdvance" : "0",
		...
		if ("200" == code) {
			...
			if (confirm("你打印的label价格为："+response.data+" 是否确定打印？")) {
				$.ajax({
					...
					//alert(code);
					if ("200" == code) {
						$("#userIdinfo [name=userName]").val("");
			            $("#userIdinfo [name=userId]").val("");
...
function initReceiverGrid() {
    ...
    receiverGrid = jQuery(receivergrid_selector).jqGrid({
        ...
        onSelectRow: function (rowid, status) {
            ...
            $("#userIdinfo [name=userName]").val(row.realName + "/" + row.phone);
            $("#userIdinfo [name=userId]").val(row.id);
            
            $("input[name='s_name']").val(row.realName);
            $("input[name='s_phone']").val(row.phone);
