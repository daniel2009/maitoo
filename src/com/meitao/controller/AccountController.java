...
public class AccountController extends BasicController {
	...
	public ResponseObject<Object> rechargeAdminOfMsd(HttpServletRequest request, AccountDetail accountDetail) {
		...
		String storeId=(String)request.getSession().getAttribute(Constant.EMP_STORE_ID_SESSION_KEY);//门店id号
		if (accountDetail != null
			...
			try {
				...
				String accountName= StringUtil.obj2String(request.getSession().getAttribute(Constant.EMP_ACCOUNT_SESSION_KEY));
				if(accountName!=null)
				{
					accountDetail.setEmpName(accountName);
				}
				String storeName = StringUtil.obj2String(request.getSession().getAttribute(Constant.EMP_STORE_NAME_SESSION_KEY));
				
				
				//Warehouse ware=this.warehouseDao.getById(storeId);
				if(storeName!=null)
				{
					accountDetail.setEmpStore(storeName);
				}
				accountDetail.setStoreId(storeId);
				accountDetail.setState(Constant.ACCOUNT_DETAIL_STATE2); // 完成
				accountDetail.setType(Constant.ACCOUNT_DETAIL_TYPE13); // 后台管理员操作
				accountDetail.setConfirm_state("0");//设置为未确认状态
				if (amount < 0) {
	...
	public void alipayReturnUrl(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			// 跳转到充值详细界面
			response.sendRedirect(request.getContextPath() + "/user/index.html");//caiwujilu.jsp
		} catch (IOException e) {
	...
	//用户端查看财务记录
	@RequestMapping(value = "/account/user/search", method = RequestMethod.GET)
	@ResponseBody
	public ResponseObject<PageSplit<AccountDetail>> searchAccountDetailByUser(
	        HttpServletRequest request,
		...
		try {
			...
			if(StringUtil.isEmpty(userId))
			{
				return new ResponseObject<PageSplit<AccountDetail>>(ResponseCode.NEED_LOGIN,"请登陆后操作！");
			}
			pageIndex = Math.max(pageIndex, 1);
			
			return this.accountService.search(userId, key, rows, pageIndex, null, type,null);
		} catch (Exception e) {
			log.error("获取预充值列表失败", e);
			return generateResponseObject(ResponseCode.SHOW_EXCEPTION, "获取预充值列表失败");
		}
	...
	//交易记录处理
	@RequestMapping(value = "/admin/account/detail_search", method = RequestMethod.GET)
	@ResponseBody
	public ResponseObject<PageSplit<AccountDetail>> searchdetailbyadmin(
			HttpServletRequest request,
	        @RequestParam(value = "userId", required = false, defaultValue = "") String userId,
		...
		try {
			...
			return this.accountService.searchbyinfo(userId, key, rows, pageIndex, state, type);
			
		} catch (Exception e) {
			log.error("获取交易列表失败", e);
			return generateResponseObject(ResponseCode.SHOW_EXCEPTION, "获取交易表失败");
		}
	}
	
	
	//信息汇总查询接口
		@RequestMapping(value = "/admin/account/detail_all_search", method = RequestMethod.GET)
		@ResponseBody
		public ResponseObject<PageSplit<AccountDetail>> searchdetailbysuperadmin(
				HttpServletRequest request,
				@RequestParam(value = "c_state", required = false, defaultValue = "") String c_state,//确认状态
				@RequestParam(value = "storeId", required = false, defaultValue = "") String storeId,//门店id
		        @RequestParam(value = "userId", required = false, defaultValue = "") String userId,
		        @RequestParam(value = "key", required = false, defaultValue = "") String key,
		        @RequestParam(value = "type", required = false, defaultValue = "") String type,
		        @RequestParam(value = "state", required = false, defaultValue = "") String state,
			...
			try {
				...
				{
					return generateResponseObject(ResponseCode.SHOW_EXCEPTION, "对不起，只有高级管理员能够查看此信息！");
				}
				...
				if(!StringUtil.isEmpty(c_state)&&(c_state.equalsIgnoreCase("-1")))
				{
					c_state=null;
				}
				...
				/*return this.accountService.search(userId, name, defaultPageSize, pageIndex, state,
				        Constant.ACCOUNT_DETAIL_TYPE11,groupid);*/
				return this.accountService.searchbyinfo_superadmin(c_state, storeId, userId, key, rows, pageIndex, state, type);
				//return this.accountService.searchbyinfo(userId, key, rows, pageIndex, state, type);
		...
		//修改确认状态
		@RequestMapping(value="/admin/account/modify_detail_confirm_state", method={RequestMethod.POST,RequestMethod.GET})
		@ResponseBody
		public ResponseObject<Object> modifydetailconfirmstate(HttpServletRequest request,
				@RequestParam(value = "id", required = false) String id,
				@RequestParam(value = "type", required = false) String type,//要修改的类型
				@RequestParam(value = "confirm_state", required = false) String confirm_state,//确认状态
				@RequestParam(value = "admin_remark", required = false) String admin_remark){//管理员备注
			try{
				if(StringUtil.isEmpty(id))
				{
					return new ResponseObject<Object>(ResponseCode.PARAMETER_ERROR,"参数id不能为空!");
				}
				if(StringUtil.isEmpty(confirm_state))
				{
					return new ResponseObject<Object>(ResponseCode.PARAMETER_ERROR,"状态参数不能为空!");
				}
				if(StringUtil.isEmpty(type))
				{
					return new ResponseObject<Object>(ResponseCode.PARAMETER_ERROR,"类型参数不能为空!");
				}
				...
				}else
				{
					return generateResponseObject(ResponseCode.NEED_LOGIN,
							"对不起，你没有登陆或不是高级管理员，无权修改此信息!");
				}
				return this.accountService.changeconfirmstate(id, type, confirm_state, admin_remark);
		...
		//共取打印清单列表
		@RequestMapping(value="/admin/account/get_detail_many", method={RequestMethod.POST,RequestMethod.GET})
		@ResponseBody
		public ResponseObject<Object> getmanyorderids(HttpServletRequest request,
			...
			try{
				...
				{
				    return new ResponseObject<Object>(ResponseCode.PARAMETER_ERROR,"对不起，你没有登陆或不是高级管理员!");
				}
				return this.accountService.getmanydetail(ids);
		...
		//获取统计数值
		@RequestMapping(value="/admin/account/get_detail_no", method={RequestMethod.POST,RequestMethod.GET})
		@ResponseBody
		public ResponseObject<Object> getdetailallno(HttpServletRequest request){
			try{
				...
				{
				    return new ResponseObject<Object>(ResponseCode.PARAMETER_ERROR,"对不起，你没有登陆或不是高级管理员,不能查看统计信息!");
				}
				return this.accountService.getdetailNo();
